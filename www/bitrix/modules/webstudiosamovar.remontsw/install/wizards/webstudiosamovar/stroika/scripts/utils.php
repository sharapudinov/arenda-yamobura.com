...
class WizardServices
{
	function GetTemplates($relativePath)
	{
		...
		while(($dirName = @readdir($handle)) !== false)
		{
			...
			if (file_exists($absolutePath."/".$dirName."/description.php"))
			{
				...
				{
					if (file_exists($absolutePath."/".$dirName."/lang/ru/description.php"))
						__IncludeLang($absolutePath."/".$dirName."/lang/ru/description.php");
				}
	...
	function IncludeServiceLang($relativePath, $lang = false, $bReturnArray = false)
	{
		...
		if ($lang != "en" && $lang != "ru")
		{
			if (file_exists(WIZARD_SERVICE_ABSOLUTE_PATH."/lang/ru/".$relativePath))
			{
				if ($bReturnArray)
					$arMessages = __IncludeLang(WIZARD_SERVICE_ABSOLUTE_PATH."/lang/ru/".$relativePath, true);
				else
					__IncludeLang(WIZARD_SERVICE_ABSOLUTE_PATH."/lang/ru/".$relativePath);
			}
		}
		...
		if ($handle = @opendir($themePath))
		{
			while (($file = readdir($handle)) !== false)
			{
				...
				if (is_file($themePath."/".$file."/description.php"))
				{
					if (LANGUAGE_ID != "en" && LANGUAGE_ID != "ru")
					{
						if (file_exists($themePath."/".$file."/lang/ru/description.php"))
							__IncludeLang($themePath."/".$file."/lang/ru/description.php");
					}
